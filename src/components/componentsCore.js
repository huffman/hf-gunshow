...
EntityManager = function(entityID, isServer, entityScript, serverEntityList) {
    ...
    if (entityScript) {
        // Mapping of entity callback methods to internal events
        var callbackToEvent = {
...
extend(EntityManager.prototype, {
    ...
    sendEvent: function(entityID, event, args) {
        ...
        if (this.isServer) {
            ...
            console.log("SendEvent", event, args);
            Entities.callEntityMethod(entityID, 'remoteEvent', [JSON.stringify([event, args])]);
        }
    },
    onRemoteEvent: function(entityID, args) {
        var parsedArgs = parseJSON(args[0]);

        if (!parsedArgs) {
            console.error("Error processing remote event, bad args", args);
            return;
        }

        var event = parsedArgs[0];
        var eventArgs = parsedArgs[1];
        console.log("Got remote event: ", event);
        console.log("Got args: ", eventArgs);
        this.emitWithArgs(event, eventArgs);
    },
    destroy: function() {
        for (var key in this.components) {
...
registerComponent = function(name, clientCtor, serverCtor) {
    ...
    console.warn("Registering component type:", name);
...
exports.hasComponent = function(entityID, componentType) {
